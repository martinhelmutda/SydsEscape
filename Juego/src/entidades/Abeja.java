...
public class Abeja extends Entidad{
	private int x,y;
	...
	public Abeja(SpriteSheet ss) {
		...
		if(direction == 0){ //compara la direccion para asignar la imagen inicial
			image = ss.grabImage(5, 5, 100, 100);
		}else{
			image = ss.grabImage(5, 4, 100, 100);
		}
	...
	public void tick() {
		...
		if(direction == 0){
			if(time%10==0){ //solo actualiza la imagen cada cierto rango, para que no vaya muy rapido
				image = ss.grabImage(cont, 5, 100, 100);
				cont++;
			}
			x -= flyingSpeed;
		}else{
			if(time%10==0){
				image = ss.grabImage(cont, 4, 100, 100);
				cont++;
			}
			x += flyingSpeed;
		}
